...
const OrderData = ({
  ...
  products,
    ...
    try {
      ...
      page.drawText("SnapCart", {
        x: 400,
        y: 750,
        size: 12,
        color: rgb(1, 1, 1),
      });
      page.drawText("Nagpur, Maharashtra", { x: 400, y: 735, size: 10 });
      page.drawText("Email: support@snapcart.com", { x: 400, y: 705, size: 10 });
  ...
  return (
    ...
    <div className=" flex sm:flex-col gap-3 sm:gap-0 mt-2 sm:mt-0 justify-end sm:items-center">
      <h2 className="text-md sm:text-xl font-bold flex items-center dark:text-customYellow ">
        <IndianRupee size={18} /> {product?.id?.price * product?.quantity}
      </h2>
  ...
  <div className="flex flex-col sm:flex-row justify-between sm:itmes-center">
     <span>Ordered On: <span className="capitalize">{new Date(createdAt).toLocaleDateString()}</span></span>
     <span onClick={handleDownloadInvoice} className="hover:underline text-sm cursor-pointer flex items-center gap-1 text-customYellow"><Download size={20}/>Download Invoice</span>
  </div>
  <hr />
  <span>Delivery On: <span className="capitalize">05 Jan 2025 </span>
    <span className="text-customYellow">| </span> {" "} Delivery At: <span className="capitalize">{address}</span>
  </span>
</Card>
